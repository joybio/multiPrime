...
def hairpin_check(primer):
	n = 0
	check = "FALSE"
	while n <= len(primer)-5-5-4:
		kmer = dege_trans(primer[n:n+5])
		left = dege_trans(primer[n+4+distance:])
		for k in kmer:
			for l in left:
...
#########################  dimer check  ################################
kmer = int(options.window)
#primer_end_set = set()
def end_dimer_check(primer_F,primer_R): #Caution: primer_key_set must be a global var.
	check = "F"
	small_end_set = set()
	small_end_set = set()
	global primer_end_set
	# construct a small set of each primer pair
#######################################################################
#######################     Method 1     ##############################
#######################################################################
	'''
	four_F_seq = dege_trans(primer_F[-4::])
	four_R_seq = dege_trans(primer_R[-4::])
	small_end_F_set = small_end_F_set.union(set(four_F_seq))
	small_end_R_set = small_end_R_set.union(set(four_R_seq))
	'''
	#######################################################################
	#######################     Method 2     ##############################
	# Strictness!!! use Carefully!!!
	n = 0
	while n < kmer - 4:
		...
		n += 1
	# compare each sequence in small set with global primer end set.
	for seq in small_end_set:
		if str(Seq(seq).reverse_complement()) in small_end_set or str(Seq(seq).complement()) in small_end_set:
			print(str(Seq(seq).reverse_complement()),small_end_set)
			check = "T"
			break
...
#		primer_end_set = primer_end_set.union(small_end_set)
#		primer_end_set = primer_end_set.union(small_end_set)
#	       print(primer_end_set)
		return False
#########################################################################
#middle_dimer_check
kmer = int(options.kmer)
def middle_dimer_check(primer_F,primer_R): #Caution: primer_middle_set must be a global var
	n = 0
	check = "F"
	primer_len = len(primer_F)
	middle_seq = set()
	primer_F = dege_trans(primer_F)
	primer_R = dege_trans(primer_R)
	while n <= primer_len-kmer:
		f,r = 0,0
		while f < len(primer_F):
		#	print(primer_F[f][n:n+kmer])
			for i in dege_trans(primer_F[f][n:n+kmer]):
				middle_seq.add(i)
			f += 1
		while r < len(primer_R):
			for x in dege_trans(primer_R[r][n:n+kmer]):
				middle_seq.add(x)
			r += 1

		n += 1
	primers = set(primer_F + primer_R)
	for seq in middle_seq:
		for R in primers:
	...
	if check == "T": #contain middle dimer
		return True
	else:
		return False	
#########################  paired primers  ################################
			...
			for primer_end in primer2speciesID_start_dict.keys():
				if primer_end - primer_start > int(user_product_size[1]): #prunding
					...
					if primer_end - primer_start >= int(user_product_size[0]) and primer_end - primer_start <= int(user_product_size[1]):
						...
						if end_dimer_check(primer_F_seq,primer_R_seq): #remove dimer
							print("Dimer primer: {} - {}. Removing...".format(primer_F_seq,primer_R_seq))
							pass
						else:
							if middle_dimer_check(primer_F_seq,primer_R_seq):
								print("Middle dimer primer: {} - {}. Removing...".format(primer_F_seq,primer_R_seq))
								pass
							else:
