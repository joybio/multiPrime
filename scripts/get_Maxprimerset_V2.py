        ...
        if len(primers[row_pointer]) <= 1:
            ...
            while column_pointer <= len(primers[row_pointer]) - step:
                if dimer_check(primers[row_pointer][column_pointer],
                    ...
                    current_forward_dict, current_reverse_dict = term9_map(ID, position, F_seq, R_seq)
...
###################################################################
def get_term9(ID, seq, out):
    global term_9bp  # 标记已经比对过的序列信息，再出现该序列信息则直接略过。
    with open(out, "w") as o:
        expand_seq = dege_trans(seq[-9:])
        for seq_rank in range(len(expand_seq)):
            tmp_ID = ID + "_" + str(seq_rank)
            sequence = expand_seq[seq_rank]
            if sequence in term_9bp:
                pass
            else:
                o.write(tmp_ID + "\n" + expand_seq[seq_rank] + "\n")
                term_9bp.add(seq_rank)
...
###################################################################
def term9_map(ID, position, F_seq, R_seq):
    ...
    if Path(term9_F).exists() and Path(term9_R).exists():
        ...
        if Path(term9_F).exists() and not Path(term9_R).exists():
            t1 = threading.Thread(target=get_term9, args=(R_ID, R_seq, term9_R))
            t1.start()
            t1.join()
            ...
            t1 = threading.Thread(target=get_term9, args=(F_ID, F_seq, term9_F))
            t1.start()
            t1.join()
            ...
            t1 = [threading.Thread(target=get_term9, args=(F_ID, F_seq, term9_F)),
                  threading.Thread(target=get_term9, args=(R_ID, R_seq, term9_R))]
            for t in t1:
                t.start()
            for t in t1:
                t.join()
