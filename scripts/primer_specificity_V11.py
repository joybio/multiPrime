...
class off_targets(object):
    def __init__(self, primer_file, term_length=9, reference_file="",
...
def main():
    ...
    primer_pairs = off_targets(primer_file=options.input_file, term_length=options.len, reference_file=options.ref,
                               PCR_product_size=options.size, outfile=options.out, nproc=options.proc)
    primer_pairs.run()
