...
def argsParse():
    ...
    parser.add_option('-m', '--method',
                      ...
                      default="T",
                      type="str",
                      help='which method: maximal or maximum. If -m [T] use maximal; else maximum. Default: T')

    parser.add_option('-c', '--coverage',
                      dest='coverage',
                      default=0.8,
                      type="float",
                      help='minimal primer coverage. default: 0.8')
...
def greedy_maximal_primers(primers, row_num, output, next_candidate, coverage):
    primer_end_set, primer_set = set(), set()
    jdict = {}
    primer_end_dict = {}
    primer_dict = {}
    ...
    while row_pointer < row_num:
        if len(primers[row_pointer]) <= 1:
            ...
            while column_pointer <= len(primers[row_pointer]) - step:
                # print(primers[row_pointer][column_pointer+2].split(":")[-1])
                if dimer_examination(primers[row_pointer][column_pointer], primers[row_pointer][column_pointer + 1],
                    ...
                    if column_pointer > len(primers[row_pointer]) - step:
                        ...
                        process_non_primer_case()
                        break
                elif float(primers[row_pointer][column_pointer+2].split(":")[-1]) < coverage:
                    print("virus {} missing!".format(primers[row_pointer][0]))
                    process_non_primer_case()
                    break
                else:
...
if __name__ == "__main__":
    ...
    if method == "T":
        ...
        with open(next_candidate, "w") as next_candidate_txt:
            greedy_maximal_primers(primers, row_num, maximal_out, next_candidate_txt, coverage)
    else:
