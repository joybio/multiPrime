...
class Dimer(object):
    ...
    def dimer_check(self, primer, primer_set):
        current_end = list(set(self.current_end(primer)))
        current_end_list = sorted(current_end, key=lambda i: len(i), reverse=True)
        dimer = False
        primers = primer_set
        for ps in primers:
                ...
                for p in self.degenerate_seq(ps):
                    ...
                    if idx >= 0:
                        ...
                        if Loss > 3 or delta_G < -5:
                            line = (self.process_dict(self.new_primer, self.core_primer)[1][primer], primer,
    ...
    def run(self):
        uniq_new_primers_dict = self.process_dict(self.new_primer, self.core_primer)[0]
        merge_primers_dict = self.process_dict(self.new_primer, self.core_primer)[1]
...
class off_targets(object):
    ...
    def build_dict(self, Input):
        Output = Path(Input).parent.joinpath(Input.stem).with_suffix(".gene_position_json")
        if Output.exists():
    ...
    def run(self):
        ...
        core_forward_dict, core_reverse_dict = self.build_dict_run(self.core_primer_file)
        new_forward_dict, new_reverse_dict = self.build_dict_run(self.new_primer_set)
...
def Offtargets_main():
    # options, args = argsParse()
    print("INFO: Start off-targets prediction ...")
